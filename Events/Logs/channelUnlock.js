...
module.exports = new Event("channelUnlock", async (bot, channel, author, reason) => {

    let logs = await bot.db.selectLogs(channel.guild.id)
    let req = await bot.db.selectGuild(channel.guild.id)

    if(!logs[0].enable.includes("channelUnlock") || req[0].channellogs === "off") return;

    let Embed = new Discord.EmbedBuilder()
    .setColor(bot.color)
    .setTitle(`${bot.function.emojis.unlock} ${req[0].lang === "fr" ? "Déverrouillage d'un salon" : "Unlock of a channel"}`)
